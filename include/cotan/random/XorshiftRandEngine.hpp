...
public:
  ...
  XorshiftRandEngine() { state_ = 2; }

  XorshiftRandEngine(const seedType state) { state_ = state; }
  ...
  void setSeed(const seedType state) noexcept { state_ = state; }

  seedType getSeed() const noexcept { return state_; }

private:
  seedType state_;
};

template <> class XorshiftRandEngine<std::uint32_t> {
public:
...
template <> class XorshiftRandEngine<std::uint64_t> {
public:
